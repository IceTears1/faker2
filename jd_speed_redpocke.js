...
function reward_query() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_query", {
      "inviter": ["zw4ZA6-snIheDOw1Y2aXeg"][Math.floor((Math.random() * 1))],
      linkId
    }), async (err, resp, data) => {
      try {
        if (err) {
...
async function redPacket() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_receive",{"inviter":["zw4ZA6-snIheDOw1Y2aXeg"][Math.floor((Math.random() * 1))], linkId}),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
